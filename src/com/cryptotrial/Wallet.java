...
public class Wallet {
    ...
    public float getBalance(){
        ...
        for (Map.Entry<String, TransactionOutput> item : Main.UTXOs.entrySet()){
            ...
            if(UTXO.isMine(publicKey)){ //if output belongs to this wallet
                UTXOs.put(UTXO.id,UTXO); //add it to wallets list
                total += UTXO.value;
            }
        }
        return total;
    ...
    //generates and returns a new transaction for this wallet
    public Transaction sendFunds(PublicKey _reciepient, float value){
        if(getBalance() < value){ //gather balance and check funds
            System.out.println("#Not Enough funds to send transactikon. Transaction discarded.  ");
            return null;
        }
